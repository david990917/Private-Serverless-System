...
mapping = {}
with open("/proxy/exec/url_idx.txt", "r") as f:
...
def getResult(url):
    filename = "/proxy/exec/{}.jpg".format(mapping[url])
    flag=False

    while not flag:
        if os.path.exists(filename): flag=True
        # else:time.sleep(0.1)
    # Retreving Photos
    ...
    total_photo_time = reading_time + processing_photo_time + inference_time
    ...
    return {"result": categories[top1_catid[0]],
            ...
            "time": {
                ...
                "photo_time": {
                    "reading_photo_time": reading_time,
