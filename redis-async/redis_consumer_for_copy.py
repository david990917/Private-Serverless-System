...
if __name__ == '__main__':
    ...
    while not flag or redis_conn.llen(target):
        if redis_conn.llen(target) and not flag:
            ...
            print("开始处理推理了", time0)
            flag = True
        if redis_conn.llen(target):
            details = json.loads(redis_conn.blpop(target)[1])
            port = 5000
            # print(mapping[details["url"].strip()],details["url"].strip())
            result = getPredictResults(port, imgUrl=details["url"].strip())
            details["result"] = result["result"]
            time_read += result["time"]["photo_time"]["reading_photo_time"]
            time_preprocess += result["time"]["photo_time"]["processing_photo_time"]
            time_inference += result["time"]["photo_time"]["inference_time"]
            count += 1
            print(count, mapping[details["url"].strip()], time.time() - time0)

    print("ALL Time:", time.time() - time0)
    print("avg_read", time_read / count)
