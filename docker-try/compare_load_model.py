...
with open("../light-network-predict/imagenet_classes.txt", "r") as f:
...
if __name__ == '__main__':
    ...
    PATH_ALL = "resnet152_all.pth"  # 241.7MB 241,740,631字节
    PATH_DICT = "resnet152_dict.pth"  # 241.7MB 241,692,183 字节
    # 保存整个网络
    ...
    time0 = time.time()
    model0 = torch.load(PATH_ALL)
    time1 = time.time()

    # model_abc = torchvision.models.resnet152()
    model_abc = resnet152()
    # print(model_abc)
    # print(type(model_abc))
    # model_abc=ResNet152()
    time2 = time.time()
    model_abc.load_state_dict(torch.load(PATH_DICT))

    time3 = time.time()

    print(time1 - time0)
    print(time3 - time1, time3 - time2, time2 - time1)
